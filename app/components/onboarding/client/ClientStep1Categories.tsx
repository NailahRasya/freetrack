...
export default function ClientStep1Categories({
  selectedCategories,
  onToggle,
}: ClientStep1Props) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ color: "rgba(226, 232, 240, 0.45)", fontSize: "14px" }}>
          Pilih satu atau lebih kategori yang sesuai dengan kebutuhan project Anda.
        </p>
      </div>
      ...
      <div style={searchWrap}>
        ...
        <input
          type="text"
          placeholder="Cari kategori (contoh: Website, Design...)"
          value={search}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px", padding: "4px" }}>
        {filteredCategories.map((cat) => {
          ...
          const isCatSelected = cat.skills.some(s => (selectedCategories || []).includes(s.id));
          return (
                  ...
                  >
                    <div style={{ padding: "16px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                      {cat.skills.map((skill) => {
                        const isSelected = (selectedCategories || []).includes(skill.id);
                        return (
                          <button
                            key={skill.id}
